<template>
  <div class="title" @touchmove.prevent>{{ text }}</div>
  <div class="canvas_box" ref="canvasBox">
    <div ref="myCanvas"></div>
  </div>
</template>
<script setup>
import { ref, onMounted, onUnmounted } from "vue"
import * as Phaser from "Phaser"

let myCanvas = ref(null)
let canvasBox = ref(null)
let game
let exampleScene
let text = ref("")

onMounted(() => {
  // This is revert of https://github.com/photonstorm/phaser/commit/7cbf3840af296c2f1f510be15b39a2519f7a72cf
  // If remove below, safari doesn't play sounds
  Phaser.Scenes.SceneManager.prototype.loadComplete = function (loader) {
    const scene = loader.scene
    if (this.game.sound && this.game.sound.onBlurPausedSounds) {
      this.game.sound.unlock()
    }
    this.create(scene)
  }
  let center = {
    x: canvasBox.value.clientWidth / 2,
    y: canvasBox.value.clientHeight / 2,
  }
  var SceneA = new Phaser.Class({
    Extends: Phaser.Scene,
    initialize: function SceneA() {
      Phaser.Scene.call(this, { key: "sceneA" })
    },
    preload: function () {
      this.load.audio("jungle", "assets/audio/jungle.mp3")
      this.load.image("wizball", "assets/wizball.png")
      this.load.bitmapFont("atari-classic", "assets/atari-classic.png", "assets/atari-classic.xml")
    },
    create() {
      console.log("SceneA")
      var text = this.add.bitmapText(center.x, 100, "atari-classic", "", 10).setOrigin(0.5)
      this.add.image(center.x, center.y, "wizball")
      var jungle = this.sound.add("jungle")
      jungle.play({ loop: true })
      if (this.sound.locked) {
        text.setText("tap to unlock\nand play music")
        this.sound.once(
          "unlocked",
          function (soundManager) {
            setupSceneInput.call(this, text, jungle)
          },
          this
        )
      } else {
        setupSceneInput.call(this, text, jungle)
      }
    },
  })

  var setupSceneInput = function (text, jungle) {
    text.setText("tap to load and play\nmusic from child scene")
    this.input.once(
      "pointerup",
      function () {
        jungle.stop()
        this.scene.start("sceneB")
      },
      this
    )
  }

  var SceneB = new Phaser.Class({
    Extends: Phaser.Scene,
    initialize: function SceneB() {
      Phaser.Scene.call(this, { key: "sceneB" })
    },
    preload: function () {
      // 该场景加载音频，到sceneC场景播放
      this.load.audio("theme", ["assets/audio/oedipus_wizball_highscore.mp3"])
    },
    create() {
      console.log("SceneB")
      this.scene.start("sceneC")
    },
  })

  var SceneC = new Phaser.Class({
    Extends: Phaser.Scene,
    initialize: function SceneC() {
      Phaser.Scene.call(this, { key: "sceneC" })
    },
    create() {
      console.log("SceneC")
      this.add.image(center.x, center.y, "wizball").setScale(4)
      // 播放在sceneB中加载的音频
      var music = this.sound.add("theme")
      music.play({ loop: true })
      if (this.sound.locked) {
        var text = this.add.bitmapText(center.x, 100, "atari-classic", "tap to unlock and play\nmusic from child scene", 10).setOrigin(0.5)
        this.sound.once(
          "unlocked",
          function (soundManager) {
            text.visible = false
          },
          this
        )
      }
    },
  })

  let config = {
    type: Phaser.AUTO,
    width: canvasBox.value.clientWidth,
    height: canvasBox.value.clientHeight,
    parent: myCanvas.value,
    backgroundColor: "#2d2d2d",
    scene: [SceneA, SceneB, SceneC],
    pixelArt: true, //将 antialias 设置为 false 并将 roundPixels 设置为 true。 这是像素艺术游戏的最佳设置
    audio: {
      // 音频配置对象
      disableWebAudio: true,
      // noAudio: false, // if this is true, it works, with audio enabled it fails
    },
  }
  game = new Phaser.Game(config)
})

onUnmounted(() => {
  for (let key in game.scene.keys) {
    if (game.scene.keys.hasOwnProperty(key)) {
      game.scene.stop(key)
      game.scene.keys[key] = undefined
    }
    // 暂停所有audio
    game.sound.stopAll()
  }
})

function changeText(value) {
  text.value = value
}
</script>
<style lang="less" scoped>
.canvas_box {
  width: 100vw;
  height: 100vh;
}
.title {
  color: #00ff00;
  position: absolute;
  width: 100%;
  display: flex;
  justify-content: center;
  margin-top: 32px;
}
</style>
